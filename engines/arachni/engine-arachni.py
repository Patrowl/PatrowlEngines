...
def _loadconfig():
    ...
    cmd = (
        ...
        + "--address "
        + this.scanner["listening_host"]
        + " --port "
        + this.scanner["listening_port"]
    ...
    app.logger.info(
        " * Arachni REST API server successfully started on http://{}:{}/".format(
            this.scanner["listening_host"], this.scanner["listening_port"]
        )
    )
...
def reloadconfig():
    ...
    res.update({"config": this.scanner, "details": {"pid": this.proc.pid}})
    return jsonify(res)
...
def info():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            res.update({"status": "ERROR", "details": {"engine_config": this.scanner}})
    except Exception:
        res.update(
            {"status": "ERROR", "details": "connexion error to the API {}".format(url)}
        )
...
def _is_scan_finished(scan_id):
    ...
    try:
        url = (
            ...
            + "/scans/"
            + str(this.scans[scan_id]["arachni_scan_id"])
            + "/summary"
        )
        r = requests.get(url=url, verify=False, auth=this.scanner["auth"])
        if (
...
def scan_status(scan_id):
    ...
    try:
        url = (
            this.scanner["api_url"]
            + "/scans/"
            + this.scans[scan_id]["arachni_scan_id"]
            + "/summary"
        )
        r = requests.get(url=url, verify=False, auth=this.scanner["auth"])
        resp = r.json()
...
def start():
    ...
    if (
        "assets" not in data.keys() or "scan_id" not in data.keys()
    ):  # or not 'base_url' in data['options'].keys():
        res.update(
            {
                "status": "ERROR",
    ...
    try:
        r = requests.post(
            url=url, data=json.dumps(post_data), verify=False, auth=this.scanner["auth"]
        )
        if r.status_code == 200:
            ...
            res.update(
                {"status": "ERROR", "reason": "something wrong with the API invokation"}
            )
            scan["status"] = "ERROR"
...
def getfindings(scan_id):
    ...
    try:
        url = (
            this.scanner["api_url"] + "/scans/" + this.scans[scan_id]["arachni_scan_id"]
        )
    ...
    with open(
        BASE_DIR + "/results/arachni_" + str(scan_id) + ".json", "w"
    ) as report_file:
        json.dump(

...
def status_scan(scan_id):
    ...
    if engine.scans[scan_id]["status"] == "ERROR":
        ...
        if engine.scans[scan_id]["ov_scan_id"] != "":
            ov_scan_status = this.openvas_cli.get_scan_status(
                engine.scans[scan_id]["ov_scan_id"]
            )
...
def _loadconfig():
    ...
    if os.path.exists(conf_file):
        ...
        if set(["omp_host", "omp_port", "omp_username", "omp_password"]).issubset(
            engine.scanner["options"].keys()
        ):
            try:
...
def _parse_results(scan_id):
    ...
    for result in report.find("results").findall("result"):
        ...
        if (
            result.find("nvt").find("cve") is not None
            and result.find("nvt").find("cve").text != "NOCVE"
        ):
            cvelist = str(result.find("nvt").find("cve").text)
            issue_meta.update({"CVE": cvelist.split(", ")})
        if (
            result.find("nvt").find("bid") is not None
            and result.find("nvt").find("bid").text != "NOBID"
        ):
            bid_list = str(result.find("nvt").find("bid").text)
            issue_meta.update({"BID": bid_list.split(", ")})
        if (
            result.find("nvt").find("xref") is not None
            and result.find("nvt").find("xref").text != "NOXREF"
        ):

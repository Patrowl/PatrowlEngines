...
def _loadconfig():
    ...
    try:
        ...
        if "access_key" in this.scanner.keys() and "secret_key" in this.scanner.keys():
            ...
            "server_username" in this.scanner.keys()
...
def getfindings(scan_id):
    ...
    if item[0]["nessus_scan_hid"] is not None:
        this.nessscan.action(
            action="scans/" + nessscan_id + "?history_id=" + item[0]["nessus_scan_hid"],
            method="GET",
        )
    else:
    ...
    nessus_prefix = "https://{}:{}".format(
        this.scanner["server_host"], this.scanner["server_port"]
    )
    # Check if FQDN shoud be resolved (default=false)
    resolve_fqdn = False
    if (
        "identifybyfqdn" in item[0]["options"].keys()
        and item[0]["options"]["identifybyfqdn"] is True
    ):
        resolve_fqdn = True
    block_summary, block_issues = parse_report(
        report_filename, nessus_prefix, resolve_fqdn
    )
    ######
    ...
    with open(BASE_DIR + "/results/nessus_" + scan_id + ".json", "w") as report_file:
        json.dump(
            {"scan": item[0], "summary": block_summary, "issues": block_issues},
            report_file,
            default=_json_serial,
        )

    res.update({"status": "success", "summary": block_summary, "issues": block_issues})
...
def start_scan():
    ...
    if post_args["options"]["action"] == "getreports":
        ...
        if (
            "getlastcompletereport" not in post_args["options"].keys()
            or post_args["options"]["getlastcompletereport"] is True
        ):
    ...
    if post_args["options"]["action"] == "scan":
        ...
        if "policy" not in post_args["options"].keys():
            res.update({"status": "error", "reason": "Missing policy name"})
            return jsonify(res)
        ...
        try:
            ...
            res.update(
                {"status": "error", "reason": "Bad policy name: {}".format(policy_name)}
            )
            return jsonify(res)

        this.nessscan.policy_set(name=policy_name)
        this.nessscan.action(
            action="policies/" + str(this.nessscan.policy_id), method="put"
        )
...
def stop_scan(scan_id):
    ...
    with transaction(table) as tr:
        tr.update(
            {"status": "STOPPED", "finished_at": int(time.time() * 1000)},
...
def stop():
    ...
    res.update({"status": "success", "details": {"timestamp": int(time.time())}})
    return jsonify(res)
...
def status():
    ...
    if table.count(Query().status == "SCANNING") == APP_MAXSCANS:
        ...
        res.update(
            {
                "status": "BUSY",
                "reason": "Max concurrent active scans reached ({})".format(
                    APP_MAXSCANS
                ),
            }
        )
    ...
    try:
        ...
        if "status" in scan.res.keys():
            ...
            res.update(
                {
                    "status": "ERROR",
                    "details": {"reason": "Nessus engine not available"},
                }
            )
    except Exception:
        this.scanner["status"] = "ERROR"
        res.update(
            {"status": "ERROR", "details": {"reason": "Nessus engine not available"}}
        )
    return jsonify(res)
...
def scan_status(scan_id):
    ...
    with transaction(table) as tr:
        try:
            this.nessscan.action(
                action="scans/" + str(item[0]["nessscan_id"]), method="GET"
            )
            ...
            if item[0]["nessus_scan_hid"] is not None:
                ...
                tr.update(
                    {"status": nessus_scan_status.upper()}, where("scan_id") == scan_id
                )

        except Exception:
            tr.update({"status": "ERROR"}, where("scan_id") == scan_id)

    return jsonify({"status": item[0]["status"], "scan": item[0]})
...
def info():
    ...
    return jsonify(
        {"page": "info", "engine_config": _without_keys(this.scanner, secret_fields)}
    )
...
def getrawreports(scan_id=None, report_format="html"):
    ...
    if not scan_id and not request.args.get("scan_id"):
        res.update(
            {"status": "error", "details": {"reason": "'scan_id' arg is missing"}}
        )
        return jsonify(res)
    ...
    this.nessscan.action(
        action="scans/{}/export/{}/status".format(scan_id, report_fileid), method="GET"
    )
    if (
        hasattr(this.nessscan.res, "status")
        and not this.nessscan.res["status"] == "ready"
    ):
        res.update({"status": "error", "details": {"reason": "report not available"}})
        return jsonify(res)

    tmp_filename = "nessus_{}_{}_{}.{}".format(
        scan_id, report_fileid, int(time.time()), report_format
    )
    with open(UPLOAD_FOLDER + "/" + tmp_filename, "wb") as handle:
        report_url = "https://{}:{}/scans/exports/{}/download".format(
            this.scanner["server_host"], this.scanner["server_port"], report_token
        )
        response = requests.get(report_url, stream=True, verify=False)

        if not response.ok:
            res.update(
                {"status": "error", "details": {"reason": "something got wrong in d/l"}}
            )
            return jsonify(res)

...
Copyright (C) 2018 Nicolas Mattiocco - @MaKyOtOx
...
def status_scan(scan_id):
    ...
    if not psutil.pid_exists(proc.pid):
        ...
        res.update({
            ...
            "info": {
                "pid": proc.pid,
                "cmd": engine.scans[scan_id]["proc_cmd"]}
        })
...
def _scan_urls(scan_id):
    ...
    engine.scans[scan_id]["report_path"] = "{}/results/{}.json".format(APP_BASE_DIR, wordpress_hostname_hash)
    wpscan_cmd += " --output '{}'".format(engine.scans[scan_id]["report_path"])
...
def _parse_results(scan_id):
    ...
    with open(engine.scans[scan_id]["report_path"], "r") as results_file:
        content = json.loads(results_file.read())


    asset = engine.scans[scan_id]["assets"][0]
...
def getfindings(scan_id):
    ...
    if not exists(engine.scans[scan_id]["report_path"]):
    ...
    try:
        ...
        res.update({"status": "error", "reason": "Report file is not a valid json"})
        return jsonify(res)
    except Exception:
        res.update({"status": "error", "reason": "Report file not readable"})
        return jsonify(res)

...
def _loadconfig():
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "error",
                "reason": "unable to convert config file to json",
            }
        else:
        ...
        if "options" not in engine.scanner:
            ...
            return {
                "status": "error",
                "reason": "you have to specify options in your config file",
            }
        for key, value in engine.scanner["options"].items():
            ...
            if value["type"] == "required" and len(value["value"]) == 0:
                ...
                return {
                    "status": "error",
                    "reason": f"you have to specify {key} in options",
                }
...
def _scan_malicious_websites(scan_id):
    ...
    if not error:
        for report in reports:
            ...
            if (
                not report["keywords"][0]["rule"].lower()
                in engine.scans[scan_id]["assets"]
            ):
                LOGGER.error(
                    f'Unable to fin asset group for {report["threat"]}: {report["keywords"][0]["rule"]} found but no match'
                )
                continue

...
def eyewitness_cmd(list_url, asset_id, scan_id, extra_opts):
    ...
    for url in list_url:
        ...
        if not screenshot_files:
            continue
        result_url = (
            "{repo_url}/{scan_id}/{asset_id}/{count}/screens/{screenshot}".format(
        ...
        report_url = "{repo_url}/{scan_id}/{asset_id}/{count}/report.html".format(
            repo_url=ENGINE.scanner["options"]["ScreenshotsURL"]["value"],
            scan_id=scan_id,
            asset_id=asset_id,
            count=count,
        )
        report_sources_path = "{base_path}/{asset_id}/{count}/source/".format(
            base_path=base_path, asset_id=asset_id, count=count
        )
        result.update(
            {
                url: {
                    "path": "{}/screens/{}".format(
                        screenshot_base_path, screenshot_files[0]
                    ),
                    "url": result_url,
                    "report": report_url,
...
def get_last_screenshot(current_path, asset_id, scan_id):
    ...
    for root, _, files in os.walk(
        ENGINE.scanner["options"]["ScreenshotsDirectory"]["value"]
    ):
    ...
    last_scan_url = (
        "{repo_url}/{scan_id}/{asset_id}/{count}/screens/{screenshot}".format(
            ...
            asset_id=asset_id,
            count=last_scan_path.split("/")[6],
            screenshot=last_scan_path.split("/")[-1],
        )
    )
...
def diff_screenshot(screenshot1, screenshot2):
    ...
    try:
        output = check_output(
            [
                ENGINE.scanner["options"]["ImageMagickComparePath"]["value"],
                "-metric",
                "RMSE",
                screenshot1,
                screenshot2,
...
def is_forsale(report_sources_path):
    ...
    if (
        not exists(report_sources_path)
        or not os.listdir(report_sources_path)
        or "RulesForSale" not in ENGINE.scanner["options"]
    ):
        return False
...
def status():
    ...
    if not exists(eyewitness_directory):
        ...
        return jsonify(
            {
                "status": "error",
                "reason": "EyeWitnessDirectory not found : {}".format(
                    eyewitness_directory
                ),
            }
        )
    ...
    if not exists(screenshots_directory):
        ...
        return {
            "status": "error",
            "reason": "ScreenshotsDirectory not found : {}".format(
                screenshots_directory
            ),
        }

    imagemagick_compare_path = ENGINE.scanner["options"]["ImageMagickComparePath"][
        "value"
    ]
    if not exists(imagemagick_compare_path):
        LOG.error(
            "Error: ImageMagickComparePath not found : %s", imagemagick_compare_path
        )
        return {
            "status": "error",
            "reason": "ImageMagickComparePath not found : {}".format(
                imagemagick_compare_path
            ),
        }
...
def _loadconfig():
    ...
    if "EyeWitnessDirectory" not in ENGINE.scanner["options"]:
        ...
        return {
            "status": "error",
            "reason": "You have to specify EyeWitnessDirectory in options",
        }
    ...
    if not exists(eyewitness_directory):
        ...
        return {
            "status": "error",
            "reason": "EyeWitnessDirectory not found : {}".format(eyewitness_directory),
        }
    ...
    if "ScreenshotsURL" not in ENGINE.scanner["options"]:
        ...
        return {
            "status": "error",
            "reason": "You have to specify ScreenshotsURL in options",
        }
    ...
    if "ScreenshotsDirectory" not in ENGINE.scanner["options"]:
        ...
        return {
            "status": "error",
            "reason": "You have to specify ScreenshotsDirectory in options",
        }
    ...
    if "ImageMagickComparePath" not in ENGINE.scanner["options"]:
        ...
        return {
            "status": "error",
            "reason": "You have to specify ImageMagickComparePath in options",
        }
    imagemagick_compare_path = ENGINE.scanner["options"]["ImageMagickComparePath"][
        "value"
    ]
    ...
    for i, asset in enumerate(assets):
        ...
        try:
            asset_data = next(
                (
                    x
                    for x in ENGINE.scans[scan_id]["assets_data"]
                    if x["value"] == asset
                ),
                None,
            )
            ...
            result = eyewitness_cmd(
                urls, asset_data["id"], scan_id, ENGINE.scans[scan_id]["options"]
            )
            LOG.warning("[%s/%s] Screenshot result: %s", i + 1, len(assets), result)

            # Get differences with the last screenshot
            for url in result:
                last_screenshot_path, last_screenshot_url = get_last_screenshot(
                    result[url]["path"], asset_data["id"], scan_id
                )
                diff = diff_screenshot(result[url]["path"], last_screenshot_path)
                LOG.warning(
                    "[%s/%s] Screenshot diff: %s percent", i + 1, len(assets), diff
                )
                result[url].update(
                    {
                        "previous_diff": diff,
            ...
            current_diff = None
            if len(result) == 2:
                current_diff = diff_screenshot(
                    result[urls[0]]["path"], result[urls[1]]["path"]
                )
            ...
            LOG.error(
                "_scan_urls: API Connexion error for asset %s: %s", asset, err_msg
            )
            return False
    ...
    for asset in ENGINE.scans[scan_id]["findings"]:
        ...
        if ENGINE.scans[scan_id]["findings"][asset]["issues"]:
            ...
            for url in asset_issues:
                ...
                is_for_sale = (
                    is_forsale(asset_issues[url]["report_sources_path"]) or is_for_sale
                )
                    ...
                    issues.append(
                        {
                            ...
                            "title": "[{}] Screenshot differs from last time".format(
                                timestamp
                            ),
                            "solution": "n/a",
                            "metadata": {
                                ...
                                "links": [
                                    asset_issues[url]["url"],
                                    asset_issues[url]["last_screenshot_url"],
                                ],
                            },
                            "type": "eyewitness_screenshot_diff",
                            "timestamp": timestamp,
                            "description": "Too much differences, Domain for sale: {}.".format(
                                is_for_sale
                            ),
                        }
                    )
                    ...
                    issues.append(
                        {
                            ...
                            "type": "eyewitness_screenshot_diff",
                            "timestamp": timestamp,
                            "description": "The difference is about {}%, Domain for sale: {}.".format(
                                previous_diff, is_for_sale
                            ),
                        }
                    )
            ...
            if asset_issues["current_diff"] is not None:
                current_diff = "The diffence between these screenshots is {}%".format(
                    asset_issues["current_diff"]
                )
            nb_vulns[get_criticity(cvss_max)] += 1
            issues.append(
                {
                    ...
                    "confidence": "certain",
                    "target": {"addr": [asset], "protocol": "http"},
                    "title": "[{}] Some domain has been screenshoted by eyewitness".format(
                        timestamp
                    ),
                    "solution": "n/a",
                    "metadata": {
                    ...
                    "timestamp": timestamp,
                    "description": "Screenshots: {}, Current Diff: {}, Domain for sale: {}".format(
                        screenshot_urls, current_diff, is_for_sale
                    ),
                }
            )
...
def getfindings(scan_id):
    ...
    with open(
        APP_BASE_DIR + "/results/eyewitness_" + scan_id + ".json", "w"
    ) as report_file:
        dump(

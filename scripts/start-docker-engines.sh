...
for engine_dir in $engines_path/*; do
  if [ -f "${engine_dir}/Dockerfile" -a -f "${engine_dir}/${engine_dir##*/}.json" ]; then
    ...
    docker run -d --rm -p $engine_port:$engine_port --name="${engine_dir##*/}-docker-001" -v $engines_path/${engine_dir##*/}/${engine_dir##*/}.json:/opt/patrowl-engines/${engine_dir##*/}/${engine_dir##*/}.json:ro patrowl-${engine_dir##*/}
    fi
done
